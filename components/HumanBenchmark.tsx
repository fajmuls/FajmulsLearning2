...
const ReactionTimeGame: React.FC<{ onFinish: (avgMs: number) => void, onExit: () => void }> = ({ onFinish, onExit }) => {
    ...
    if (state === 'WAITING') { content = <><Zap size={64} className="mx-auto mb-4 text-amber-400"/><h2 className="text-2xl sm:text-3xl font-bold text-white mb-1">Reaction Time</h2><h3 className="text-base text-white/60 mb-6">Percobaan {attempts.length + 1} dari 5</h3><p className="text-sm text-white/80">Klik untuk mulai.</p></>; bgColor = 'bg-slate-800'; }
    else if (state === 'READY') { content = <h2 className="text-4xl font-bold text-white tracking-widest">...</h2>; bgColor = 'bg-rose-600'; }
    else if (state === 'GO') { content = <h2 className="text-5xl font-black text-white">KLIK!</h2>; bgColor = 'bg-emerald-500'; }
    else if (state === 'RESULT') { content = <><div className="text-xl text-white/80 mb-1">{currentReaction} ms</div><p className="text-sm text-white/60">Klik untuk lanjut.</p></>; bgColor = 'bg-indigo-600'; }
    else if (state === 'FINAL_SCORE') { content = <><Activity size={48} className="mx-auto mb-3 text-emerald-400"/><div className="text-lg text-white/80 mb-1">Rata-rata</div><h2 className="text-5xl font-black text-white mb-6">{average} ms</h2><div className="flex flex-col gap-2 w-full max-w-[240px] mx-auto"><button className="py-3 bg-white text-slate-900 rounded-xl font-bold text-sm" onClick={() => onFinish(average)}>Simpan Skor</button><button className="py-3 border-2 border-white/30 text-white rounded-xl font-bold text-sm" onClick={() => { setAttempts([]); setState('WAITING'); }}>Coba Lagi</button></div></>; bgColor = 'bg-slate-800'; }
    else if (state === 'TOO_SOON') { content = <><Info size={48} className="mx-auto mb-3 text-rose-400"/><h2 className="text-2xl font-bold text-white mb-3">Terlalu Cepat!</h2><p className="text-sm text-white/80">Klik untuk ulang.</p></>; bgColor = 'bg-slate-800'; }
...
const SequenceMemoryGame: React.FC<{ onFinish: (level: number) => void, onExit: () => void }> = ({ onFinish, onExit }) => {
    ...
    if (gameState === 'START') return (<div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center text-white p-4"><button onClick={onExit} className="absolute top-6 left-6 bg-white/10 p-3 rounded-full text-white z-[60]"><ArrowLeft size={24}/></button><div className="text-center max-w-sm"><Grid size={64} className="mx-auto mb-4 text-indigo-400"/><h2 className="text-2xl sm:text-3xl font-bold mb-2">Sequence Memory</h2><p className="text-white/60 mb-6 text-sm">Ingat urutan kotak yang menyala. Pola akan bertambah satu langkah setiap level.</p><button onClick={() => startLevel(1, [])} className="w-full max-w-[200px] py-3 bg-indigo-500 text-white rounded-xl font-bold text-base hover:bg-indigo-600 transition">Mulai</button></div></div>);
    if (gameState === 'GAMEOVER') return <div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center text-white p-4"><div className="bg-slate-800 p-6 sm:p-8 rounded-3xl text-center shadow-2xl max-w-xs w-full"><Grid size={48} className="mx-auto mb-4 text-rose-500"/><h2 className="text-2xl font-bold mb-1">Game Over</h2><p className="text-slate-400 mb-6 text-sm">Level: <span className="text-white font-bold text-lg">{level}</span></p><div className="space-y-2"><button onClick={() => { setLevel(1); startLevel(1, []); }} className="w-full py-3 bg-indigo-600 rounded-xl font-bold text-sm">Coba Lagi</button><button onClick={() => onFinish(Math.max(1, level - 1))} className="w-full py-3 border border-slate-600 rounded-xl font-bold text-sm">Simpan Skor</button></div></div></div>;
    return <div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center p-4"><button onClick={onExit} className="absolute top-6 left-6 bg-white/10 p-3 rounded-full text-white"><ArrowLeft size={24}/></button><div className="text-center mb-6"><div className="text-xl font-bold text-white mb-0.5">Level {level}</div><p className="text-slate-400 text-xs animate-pulse h-4">{gameState === 'WATCHING' ? 'Lihat polanya...' : 'Giliranmu!'}</p></div><div className="grid grid-cols-3 gap-2 sm:gap-3 p-2">{Array.from({length: 9}).map((_, i) => (<div key={i} onClick={() => handleTileClick(i)} className={`w-20 h-20 sm:w-28 sm:h-28 rounded-xl transition-all duration-100 cursor-pointer border-4 border-slate-800 ${activeTile === i ? 'bg-white shadow-[0_0_20px_rgba(255,255,255,0.6)] scale-95 border-white' : 'bg-slate-700 hover:bg-slate-600'}`}/>))}</div></div>;
};
...
const AimTrainerGame: React.FC<{ onFinish: (avgMs: number) => void, onExit: () => void }> = ({ onFinish, onExit }) => {
    ...
    return (<div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center select-none" onMouseDown={handleMiss}>
        ...
        {state === 'IDLE' && (
            <div className="text-center text-white cursor-default p-4 max-w-xs" onClick={(e) => e.stopPropagation()}>
                <Target size={64} className="mx-auto mb-4 text-orange-400"/>
                <h2 className="text-2xl sm:text-3xl font-bold mb-2">Aim Trainer</h2>
                <p className="text-white/60 mb-6 text-sm">Klik 30 target secepat mungkin. Waktu dimulai saat target pertama diklik.</p>
                <button onClick={startGame} className="w-full py-3 bg-orange-500 text-white rounded-xl font-bold text-base hover:bg-orange-600 transition">Mulai</button>
            </div>
        )}
        {state === 'PLAYING' && (
            <>
        ...
        {state === 'FINISHED' && (
            <div className="text-center text-white cursor-default p-4 bg-slate-800 rounded-3xl border border-slate-700 max-w-xs w-full" onClick={(e) => e.stopPropagation()}>
                <Target size={48} className="mx-auto mb-3 text-emerald-400"/>
                <h2 className="text-2xl font-bold mb-1">Selesai!</h2>
                <p className="text-slate-400 mb-4 text-xs">Rata-rata waktu per target</p>
                <h1 className="text-5xl font-black mb-6 text-orange-400">{avgTime} ms</h1>
                <div className="space-y-2 w-full">
                    <button onClick={() => onFinish(avgTime)} className="w-full py-3 bg-white text-slate-900 rounded-xl font-bold text-sm">Simpan Skor</button>
                    <button onClick={startGame} className="w-full py-3 border border-white/20 rounded-xl font-bold text-sm">Coba Lagi</button>
                </div>
            </div>
        )}
    </div>);
...
const ChimpTestGame: React.FC<{ onFinish: (score: number) => void, onExit: () => void }> = ({ onFinish, onExit }) => {
    ...
    return (<div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center select-none p-4"><button onClick={onExit} className="absolute top-6 left-6 bg-white/10 p-3 rounded-full text-white z-[60]"><ArrowLeft size={24}/></button>{state === 'IDLE' && (<div className="text-center text-white max-w-sm"><Brain size={64} className="mx-auto mb-4 text-teal-400"/><h2 className="text-2xl sm:text-3xl font-bold mb-2">Chimp Test</h2><p className="text-white/60 mb-6 text-sm">Klik angka secara berurutan. Setelah klik angka 1, sisanya akan tertutup. Uji ingatan visualmu!</p><button onClick={() => generateLevel(4)} className="w-full max-w-[200px] py-3 bg-teal-500 text-white rounded-xl font-bold text-base hover:bg-teal-600 transition">Mulai Tes</button></div>)}{state === 'PLAYING' && (<><div className="text-white mb-3 font-bold text-base">Level {level - 3}</div><div className="grid grid-cols-5 sm:grid-cols-8 gap-1.5 sm:gap-3 p-2 max-w-full">{Array.from({length: 40}).map((_, i) => { const num = numbers.find(n => n.pos === i); if (!num || !num.visible) return <div key={i} className="w-8 h-8 sm:w-12 sm:h-12"></div>; return (<button key={i} onMouseDown={() => handleClick(num.id)} className={`w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 rounded-lg border font-black flex items-center justify-center transition-transform active:scale-90 text-lg sm:text-xl md:text-2xl ${hideNumbers && num.id > 1 ? 'bg-indigo-100 border-indigo-200 text-transparent' : 'bg-indigo-500 border-indigo-400 text-white shadow-md'}`}>{hideNumbers && num.id > 1 ? '' : num.id}</button>); })}</div></>)}{state === 'GAMEOVER' && (<div className="text-center text-white p-4 bg-slate-800 rounded-3xl border border-slate-700 max-w-xs w-full"><Brain size={48} className="mx-auto mb-3 text-rose-500"/><h2 className="text-2xl font-bold mb-1">Game Over</h2><p className="text-white/60 mb-6 text-sm">Skor: <span className="text-white font-bold text-lg">{level - 1}</span></p><div className="space-y-2 w-full"><button onClick={() => { generateLevel(4); setLevel(4); }} className="w-full py-3 bg-indigo-600 rounded-xl font-bold text-sm">Coba Lagi</button><button onClick={() => onFinish(Math.max(0, level - 1))} className="w-full py-3 border border-white/20 rounded-xl font-bold text-sm">Simpan Skor</button></div></div>)}</div>);
};
...
const VisualMemoryGame: React.FC<{ onFinish: (score: number) => void, onExit: () => void }> = ({ onFinish, onExit }) => {
    ...
    if (state === 'START') return (<div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center text-white p-4"><button onClick={onExit} className="absolute top-6 left-6 bg-white/10 p-3 rounded-full text-white z-[60]"><ArrowLeft size={24}/></button><div className="text-center max-w-xs"><Grid size={64} className="mx-auto mb-4 text-sky-400"/><h2 className="text-2xl sm:text-3xl font-bold mb-2">Visual Memory</h2><p className="text-white/60 mb-6 text-sm">Ingat posisi kotak putih yang muncul. Hati-hati, setiap kesalahan mengurangi nyawa!</p><button onClick={() => { setLevel(1); setLives(3); startLevel(1); }} className="w-full max-w-[200px] py-3 bg-sky-500 text-white rounded-xl font-bold text-base hover:bg-sky-600 transition">Mulai</button></div></div>);
    if (state === 'GAMEOVER') return (<div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center text-white p-4"><div className="bg-slate-800 p-6 sm:p-8 rounded-3xl text-center shadow-2xl max-w-xs w-full border border-slate-700"><Grid size={48} className="mx-auto mb-4 text-sky-500"/><h2 className="text-2xl font-bold mb-1">Game Over</h2><p className="text-slate-400 mb-6 text-sm">Level: <span className="text-white font-bold text-lg">{level}</span></p><div className="space-y-2"><button onClick={() => { setLevel(1); setLives(3); startLevel(1); }} className="w-full py-3 bg-indigo-600 rounded-xl font-bold text-sm">Coba Lagi</button><button onClick={() => onFinish(level)} className="w-full py-3 border border-slate-600 rounded-xl font-bold text-sm text-white">Simpan Skor</button></div></div></div>);
    return (<div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center p-4"><button onClick={onExit} className="absolute top-6 left-6 bg-white/10 p-3 rounded-full text-white"><ArrowLeft size={24}/></button><div className="text-center mb-6 flex flex-col items-center gap-1"><div className="text-xl font-bold text-white">Level {level}</div><div className="flex gap-1">{[...Array(3)].map((_, i) => (<Heart key={i} size={20} className={i < lives ? "fill-rose-500 text-rose-500" : "fill-slate-700 text-slate-700"} />))}</div></div><div className="grid gap-1.5 bg-slate-800 p-3 rounded-xl shadow-2xl transition-all duration-300" style={{ gridTemplateColumns: `repeat(${gridSize}, 1fr)`, width: 'min(90vw, 360px)', aspectRatio: '1/1' }}>{Array.from({length: gridSize * gridSize}).map((_, i) => { const isActive = activeTiles.includes(i); const isClicked = userClicks.includes(i); const isWrong = wrongClicks.includes(i); let bgClass = 'bg-slate-700 hover:bg-slate-600'; if (state === 'SHOWING' && isActive) bgClass = 'bg-white shadow-[0_0_10px_white] scale-95'; if (state === 'PLAYING' && isClicked) bgClass = 'bg-white'; if (state === 'PLAYING' && isWrong) bgClass = 'bg-slate-900 border border-rose-900'; return (<div key={i} onMouseDown={() => handleTileClick(i)} className={`rounded-md transition-all duration-200 cursor-pointer ${bgClass}`}></div>); })}</div></div>);
};
...
const NumberMemoryGame: React.FC<{ onFinish: (level: number) => void, onExit: () => void }> = ({ onFinish, onExit }) => {
    ...
    if (state === 'START') return (<div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center text-white p-4"><button onClick={onExit} className="absolute top-6 left-6 bg-white/10 p-3 rounded-full text-white z-[60]"><ArrowLeft size={24}/></button><div className="text-center max-w-xs"><Hash size={64} className="mx-auto mb-4 text-blue-400"/><h2 className="text-2xl sm:text-3xl font-bold mb-2">Number Memory</h2><p className="text-white/60 mb-6 text-sm">Ingat angka yang muncul, lalu ketik ulang. Angka akan bertambah panjang setiap level.</p><button onClick={startGame} className="w-full max-w-[200px] py-3 bg-blue-500 text-white rounded-xl font-bold text-base hover:bg-blue-600 transition">Mulai</button></div></div>);
    if (state === 'GAMEOVER') return (<div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center text-white p-4"><div className="bg-slate-800 p-6 sm:p-8 rounded-3xl text-center shadow-2xl max-w-xs w-full"><Hash size={48} className="mx-auto mb-4 text-blue-500"/><h2 className="text-2xl font-bold mb-1">Salah!</h2><div className="text-left bg-slate-900 p-3 rounded-xl mb-4 font-mono text-xs break-all"><div className="text-slate-400 mb-0.5">Angka:</div><div className="text-white font-bold mb-2 tracking-widest">{number}</div><div className="text-slate-400 mb-0.5">Jawab:</div><div className="text-rose-500 line-through tracking-widest">{input}</div></div><p className="text-slate-400 mb-6 text-sm">Level: <span className="text-white font-bold">{level}</span></p><div className="space-y-2"><button onClick={startGame} className="w-full py-3 bg-indigo-600 rounded-xl font-bold text-sm">Coba Lagi</button><button onClick={() => onFinish(level)} className="w-full py-3 border border-slate-600 rounded-xl font-bold text-sm text-white">Simpan Skor</button></div></div></div>);
    
    return (<div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center p-4"><button onClick={onExit} className="absolute top-6 left-6 bg-white/10 p-3 rounded-full text-white"><ArrowLeft size={24}/></button>
        {state === 'SHOWING' && (
            <div className="text-center animate-fade-in-up w-full px-6">
                <h2 className="text-4xl sm:text-6xl md:text-8xl font-black text-white font-mono tracking-widest break-all">{number}</h2>
                <div className="w-full max-w-xs h-1.5 bg-slate-800 rounded-full mt-6 mx-auto overflow-hidden">
        ...
        {state === 'INPUT' && (<div className="text-center w-full max-w-xs p-4"><h3 className="text-lg text-white mb-4">Apa angkanya?</h3><form onSubmit={handleSubmit}><input type="text" inputMode="numeric" pattern="[0-9]*" autoFocus value={input} onChange={e => setInput(e.target.value)} className="w-full bg-slate-800 text-white text-2xl font-mono text-center py-3 rounded-xl border-2 border-slate-700 focus:border-blue-500 focus:outline-none mb-4"/><button type="submit" className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition text-sm">Submit</button></form></div>)}{state === 'RESULT' && (<div className="text-center"><CheckCircle size={64} className="text-emerald-500 mx-auto mb-3 animate-bounce-slow"/><h2 className="text-xl text-white font-bold">Benar!</h2></div>)}</div>);
};
...
const VerbalMemoryGame: React.FC<{ onFinish: (score: number) => void, onExit: () => void }> = ({ onFinish, onExit }) => {
    ...
    if (gameState === 'START') return (<div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center text-white p-4"><button onClick={onExit} className="absolute top-6 left-6 bg-white/10 p-3 rounded-full text-white z-[60]"><ArrowLeft size={24}/></button><div className="text-center max-w-xs"><Type size={64} className="mx-auto mb-4 text-purple-400"/><h2 className="text-2xl sm:text-3xl font-bold mb-2">Verbal Memory</h2><p className="text-white/60 mb-6 text-sm">Jika kata belum pernah muncul, klik <strong>BARU</strong>. Jika sudah, klik <strong>SUDAH</strong>.</p><button onClick={startGame} className="w-full max-w-[200px] py-3 bg-purple-500 text-white rounded-xl font-bold text-base hover:bg-purple-600 transition">Mulai</button></div></div>);
    if (gameState === 'GAMEOVER') return (<div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center text-white p-4"><div className="bg-slate-800 p-6 sm:p-8 rounded-3xl text-center shadow-2xl max-w-xs w-full border border-slate-700"><Type size={48} className="mx-auto mb-4 text-purple-500"/><h2 className="text-2xl font-bold mb-1">Game Over</h2><p className="text-slate-400 mb-6 text-sm">Skor: <span className="text-white font-bold text-lg">{score}</span> kata</p><div className="space-y-2"><button onClick={startGame} className="w-full py-3 bg-indigo-600 rounded-xl font-bold text-sm">Coba Lagi</button><button onClick={() => onFinish(score)} className="w-full py-3 border border-slate-600 rounded-xl font-bold text-sm text-white">Simpan Skor</button></div></div></div>);
    return (<div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center p-4"><button onClick={onExit} className="absolute top-6 left-6 bg-white/10 p-3 rounded-full text-white"><ArrowLeft size={24}/></button><div className="flex gap-6 mb-8 text-base font-bold text-white items-center"><div>Skor: {score}</div><div className="flex gap-1">{[...Array(3)].map((_, i) => (<Heart key={i} size={20} className={i < lives ? "fill-rose-500 text-rose-500" : "fill-slate-700 text-slate-700"} />))}</div></div><div className="text-4xl sm:text-6xl font-black text-white mb-10 text-center px-4">{currentWord}</div><div className="flex gap-3 w-full max-w-xs"><button onClick={() => handleAnswer('SEEN')} className="flex-1 py-4 bg-amber-500 text-white rounded-xl font-bold text-lg">SUDAH</button><button onClick={() => handleAnswer('NEW')} className="flex-1 py-4 bg-purple-500 text-white rounded-xl font-bold text-lg">BARU</button></div></div>);
};
...
const TypingGame: React.FC<{ onFinish: (wpm: number) => void, onExit: () => void }> = ({ onFinish, onExit }) => {
    ...
    if (state === 'FINISHED') return (<div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center text-white p-4"><div className="bg-slate-800 p-6 sm:p-8 rounded-3xl text-center shadow-2xl max-w-xs w-full border border-slate-700"><FileText size={48} className="mx-auto mb-4 text-slate-400"/><h2 className="text-2xl font-bold mb-1">Typing Speed</h2><h1 className="text-5xl font-black text-emerald-400 mb-6">{wpm} <span className="text-lg text-white/50">WPM</span></h1><div className="space-y-2"><button onClick={() => onFinish(wpm)} className="w-full py-3 bg-white text-slate-900 rounded-xl font-bold text-sm">Simpan Skor</button><button onClick={startGame} className="w-full py-3 border border-slate-600 rounded-xl font-bold text-sm">Coba Lagi</button></div></div></div>);
    return (<div className="fixed inset-0 z-50 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center p-4"><button onClick={onExit} className="absolute top-6 left-6 bg-white/10 p-3 rounded-full text-white"><ArrowLeft size={24}/></button><div className="flex gap-8 mb-6 text-white font-mono text-base"><div>WPM: <span className="font-bold text-emerald-400">{wpm}</span></div><div>Time: <span className="font-bold text-emerald-400">{timeElapsed}s</span></div></div><div className="w-full max-w-lg bg-slate-800 p-4 rounded-2xl shadow-xl border border-slate-700"><div className="mb-4 text-base text-slate-400 leading-relaxed select-none h-32 overflow-y-auto px-2">{text.split('').map((char, i) => { let color = 'text-slate-500'; if (i < input.length) { color = input[i] === char ? 'text-white' : 'text-rose-500 bg-rose-500/20'; } return <span key={i} className={color}>{char}</span>; })}</div><textarea value={input} onChange={handleChange} className="w-full bg-slate-900 text-white p-3 rounded-xl border border-slate-700 focus:border-indigo-500 focus:outline-none h-24 resize-none font-mono text-sm" placeholder="Mulai ketik di sini..." autoFocus/></div></div>);
};
...
const ArticleTestGame: React.FC<{ onFinish: (score: number) => void, onExit: () => void }> = ({ onFinish, onExit }) => {
    ...
    if (state === 'START') {
        return (
            <div className="w-full max-w-sm mx-auto bg-slate-900 border border-slate-700 rounded-3xl overflow-hidden shadow-2xl flex flex-col p-6 text-center animate-fade-in text-white relative">
                <button onClick={onExit} className="absolute top-4 left-4 p-2 text-slate-400 hover:text-white transition bg-slate-800 rounded-full"><ArrowLeft size={18}/></button>
                <div className="w-16 h-16 bg-orange-500/20 text-orange-400 rounded-full flex items-center justify-center mx-auto mb-4 text-orange-500"><FileText size={32}/></div>
                <h2 className="text-xl sm:text-2xl font-black mb-3 text-orange-500">Tes Pasal (Hukum)</h2>
                <p className="text-slate-400 mb-6 text-sm max-w-xs mx-auto">Uji pemahamanmu tentang hukum dan pasal-pasal di Indonesia.</p>
                <div className="grid grid-cols-1 gap-3">
                    <button onClick={() => startGame('GENERAL')} className="w-full py-3 bg-orange-600 hover:bg-orange-500 text-white rounded-xl font-black text-base shadow-lg transition-all active:scale-95 flex flex-col items-center">
                        <span>Mode Keseluruhan</span>
                        <span className="text-[10px] font-normal opacity-80">50 Soal Umum</span>
                    </button>
                    <button onClick={() => startGame('TWK')} className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-black text-base shadow-lg transition-all active:scale-95 flex flex-col items-center">
                        <span>Mode Khusus TWK</span>
                        <span className="text-[10px] font-normal opacity-80">30 Soal Bela Negara</span>
                    </button>
                </div>
            </div>
        );
    ...
    if (state === 'LOADING') {
        return (
            <div className="w-full max-w-sm mx-auto h-[400px] bg-slate-900 border border-slate-700 rounded-3xl flex flex-col items-center justify-center animate-fade-in text-white relative">
                <Loader2 className="animate-spin text-orange-500 mb-4" size={32}/>
                <h2 className="text-lg font-bold">Mengambil Data...</h2>
            </div>
        );
    }
    ...
    if (state === 'PLAYING' || state === 'EXPLANATION') {
        ...
        return (
            <div className="w-full max-w-sm mx-auto min-h-[500px] bg-slate-900 border border-slate-700 rounded-3xl flex flex-col animate-fade-in text-white relative p-4">
                <button onClick={onExit} className="absolute top-4 left-4 p-2 text-slate-400 hover:text-white transition bg-slate-800 rounded-full z-10"><ArrowLeft size={16}/></button>
                
                <div className="flex justify-between items-center mb-6 px-2">
                    <div className="text-slate-400 font-bold uppercase tracking-wider text-[10px] flex items-center gap-1.5">Skor <span className="text-lg text-orange-400">{score}</span></div>
                    <div className="flex gap-0.5">
                        {[1,2,3].map(i => (
                            <Heart key={i} size={18} className={i <= lives ? "text-rose-500 fill-rose-500" : "text-slate-700 fill-slate-700"} />
                        ))}
                    </div>
                </div>

                <div className="flex-1 flex flex-col items-center w-full">
                    <div className="text-orange-400 font-bold tracking-widest uppercase mb-1 text-[10px]">Soal {qIndex + 1}/{questions.length}</div>
                    <div className="text-base sm:text-lg font-bold mb-6 text-center leading-snug px-2">{q.question}</div>
                    ...
                    <div className="grid grid-cols-1 gap-2 w-full mb-6">
                        {q.options.map((opt: string, i: number) => {
                            ...
                            if (state === 'EXPLANATION') {
                                ...
                                style = "bg-slate-800 hover:bg-slate-700 border-slate-700 active:border-orange-500";
                            }
                            ...
                            return (
                                <button
                                    ...
                                    onClick={() => handleAnswer(opt)}
                                    className={`p-3 border rounded-xl text-left font-bold text-xs transition-all ${style} flex items-center gap-3`}
                                >
                                    <div className={`w-6 h-6 shrink-0 rounded-md flex items-center justify-center text-[10px] ${isSelected ? 'bg-orange-500 text-white' : 'bg-slate-700 text-slate-300'}`}>
                        ...
                        <div className="w-full animate-slide-up">
                            <div className="bg-slate-800 border border-slate-700 rounded-xl p-4 mb-4">
                                <div className="flex items-center gap-2 mb-1.5">
                                    ...
                                    <span className="font-bold text-orange-400 uppercase text-[9px] tracking-wider">Pembahasan: {q.article}</span>
                                </div>
                                <p className="text-slate-300 text-[11px] leading-normal">{q.explanation}</p>
                            </div>
                            <button
                                onClick={handleNext}
                                className="w-full py-3 bg-orange-600 text-white rounded-xl font-black text-sm transition-all shadow-lg"
                            >
                                {lives <= 0 ? 'Lihat Hasil' : 'Lanjut'}
                            </button>
                        </div>
                    )}
                </div>
    ...
    return (
        <div className="w-full max-w-sm mx-auto h-[400px] bg-slate-900 border border-slate-700 rounded-3xl flex flex-col items-center justify-center animate-fade-in text-white relative p-6">
            <Trophy size={48} className="mb-4 text-orange-500"/>
            <h2 className="text-2xl font-black mb-2">Selesai!</h2>
            <div className="text-sm text-slate-400 mb-1">Skor Kamu</div>
            <div className="text-5xl font-black text-orange-400 mb-6">{score}</div>
            <button onClick={onExit} className="px-8 py-2 bg-slate-800 rounded-xl font-bold text-sm">Kembali</button>
        </div>
    );
};
