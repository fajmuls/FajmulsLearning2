...
export const TesKecermatan: React.FC<TesKecermatanProps> = ({ initialMode, onBack, onComplete }) => {
    ...
    const finishTest = () => {
        ...
        const historyItem: TestHistoryItem = {
            ...
            category: 'KECERMATAN',
            packageTitle: `Tes Kecermatan: ${mode.replace(/_/g, ' ').split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ')}`,
    ...
    const handleEarlyExit = () => {
        ...
        const historyItem: TestHistoryItem = {
            ...
            date: new Date().toISOString(),
            category: 'KECERMATAN',
            packageTitle: `Tes Kecermatan: ${mode.replace(/_/g, ' ').split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ')} (Berhenti)`,
            score: Math.round(accuracy), 
            maxScore: 100,
            details: resultDetails,
            questions: [],
